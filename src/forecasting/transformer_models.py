...
class TransformerForecaster(BaseForecaster):
    ...
    def _build_model(self, input_shape):
        ...
        # Layer Transformer semplificati
...
class TimesFMForecaster(BaseForecaster):
    ...
    def _build_model(self, input_shape):
        ...
        # Proiezione iniziale (simula patching semplificato)
        x = Dense(self.d_model, activation='relu')(inputs)
        
        # Stack di layer attention (architettura foundation model style)
        for i in range(self.num_layers):
            # Self-attention
            attention_output = MultiHeadAttention(
                num_heads=self.num_heads,
            ...
            # Feed-forward con espansione (stile foundation model)
            ff_output = Dense(self.d_model * 4, activation='gelu')(x)  # GELU come TimesFM
            ff_output = Dense(self.d_model)(ff_output)
